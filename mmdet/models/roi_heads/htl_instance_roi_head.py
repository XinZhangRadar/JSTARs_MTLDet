...
from mmdet.core import ArrayMasks
import cv2
import numpy as np
import os
@HEADS.register_module()
class HybridTaskLearningInstanceCascadeRoIHead(CascadeRoIHead):
    ...
    def __init__(self,
                 ...
                 edge_head=None,
                 edge_fusion=('bbox'),
                 denisity_mask_pred_head = None,
                 edge_mask_pred_head = None,
                 interleaved=True,
                 mask_info_flow=True,
                 attention_fusion = True,
                 **kwargs):
        super(HybridTaskLearningInstanceCascadeRoIHead,
        ...
        if denisity_head is not None:
            ...
            self.denisity_mask_pred_head = build_head(denisity_mask_pred_head)
        ...
        if edge_head is not None:
            ...
            self.edge_mask_pred_head = build_head(edge_mask_pred_head)
            self.with_edge = True
    ...
    def init_weights(self, pretrained):
        ...
        Args:
            pretrained (str, optional): Path to pre-trained weights.
                Defaults to None.
        """
        super(HybridTaskLearningInstanceCascadeRoIHead, self).init_weights(pretrained)
    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            if self.with_denisity or self.with_edge:
                ...
                if self.interleaved:
                    ...
                    with torch.no_grad():
                        ...
                        for j in range(num_imgs):
                            ...
                            sampling_results.append(sampling_result)
                if self.with_denisity:
                    denisity_results = self._den_forward_train(
                        i, denisity_feat, sampling_results, gt_den, rcnn_train_cfg)
                    for name, value in denisity_results.items():
                        losses[f's{i}.{name}'] = (
                            value * lw if 'loss' in name else value)
                if self.with_edge:
                    edge_results = self._edge_forward_train(
                        i, edge_feat, sampling_results, gt_edge, rcnn_train_cfg)
                    for name, value in edge_results.items():
                        losses[f's{i}.{name}'] = (
                            value * lw if 'loss' in name else value)
        ...
        return losses
    def _den_forward_train(self,
                            stage,
                            denisity_feat,
                            sampling_results,
                            gt_dens,
                            rcnn_train_cfg):
        """Run forward function and calculate loss for den head in
        training."""
        

        denisity_roi_extractor = self.denisity_roi_extractor
        denisity_mask_pred_head = self.denisity_mask_pred_head
        pos_rois = bbox2roi([res.pos_bboxes for res in sampling_results])
        den_feats = denisity_roi_extractor([denisity_feat],pos_rois)


        # mask information flow
        # forward all previous mask heads to obtain last_feat, and fuse it
        # with the normal den feature

        den_pred = denisity_mask_pred_head(den_feats)
        #import pdb;pdb.set_trace()
        _gt_den = []

        for gt_den in gt_dens:
            _gt_den.append(ArrayMasks(gt_den,gt_den.shape[1],gt_den.shape[2]))
        #import pdb;pdb.set_trace()
        den_targets = denisity_mask_pred_head.get_targets(sampling_results, _gt_den,
                                             rcnn_train_cfg)

        #cv2.imwrite('gt_den.jpg',den_targets[0].detach().cpu().numpy()*255)
        #cv2.imwrite('pred_den.jpg',den_pred[0][0].detach().cpu().numpy()*255)
        pos_labels = torch.cat([res.pos_gt_labels for res in sampling_results])
        loss_den = denisity_mask_pred_head.loss(den_pred, den_targets, pos_labels)

        den_results = dict(loss_denisity=loss_den['loss_mask'])
        return den_results

    def _edge_forward_train(self,
                             stage,
                             edge_feat,
                             sampling_results,
                             gt_edges,
        ...
        edge_roi_extractor = self.edge_roi_extractor
        edge_mask_pred_head = self.edge_mask_pred_head
        pos_rois = bbox2roi([res.pos_bboxes for res in sampling_results])
        edge_feats = edge_roi_extractor([edge_feat],pos_rois)

        edge_pred = edge_mask_pred_head(edge_feats)
        #import pdb;pdb.set_trace()
        _gt_edge = []

        for gt_edge in gt_edges:
            _gt_edge.append(ArrayMasks(gt_edge,gt_edge.shape[1],gt_edge.shape[2]))
        #import pdb;pdb.set_trace()
        edge_targets = edge_mask_pred_head.get_targets(sampling_results, _gt_edge,
                                             rcnn_train_cfg)

        pos_labels = torch.cat([res.pos_gt_labels for res in sampling_results])
        loss_edge = edge_mask_pred_head.loss(edge_pred, edge_targets, pos_labels)

        edge_results = dict(loss_edge=loss_edge['loss_mask'])
        return edge_results
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        if self.with_denisity:
            ...
            if self.attention_fusion:
                denisity_attention = torch.sigmoid(self.attention_conv_denisity(denisity_feat))
                denisity_feat = denisity_feat.mul(denisity_attention)    
            denisity_pred = self.denisity_mask_pred_head(denisity_feat)
            #self.save_density_map(denisity_pred[0][0].detach().cpu().numpy(),'/share/home/zhangxin/mmdetection/vis/HRSID/task/denisity_pred.jpg')
        ...
        if self.with_edge:
            ...
            if self.attention_fusion:
                edge_attention = torch.sigmoid(self.attention_conv_edge(edge_feat))
                edge_feat = edge_feat.mul(edge_attention)  
            edge_pred = self.edge_mask_pred_head(edge_feat)
            #cv2.imwrite('edge_pred.jpg',edge_pred[0][0].detach().cpu().numpy()*255)
            #self.save_density_map(edge_pred[0][0].detach().cpu().numpy(),'/share/home/zhangxin/mmdetection/vis/HRSID/task/edge_pred.jpg')  
        else:
            edge_feat = None
            edge_pred = None
    ...
    def save_density_map(self,density_map, output_dir = '/share/home/zhangxin/mmdetection/mmdet/datasets/pipelines/HTL/results.png'):
